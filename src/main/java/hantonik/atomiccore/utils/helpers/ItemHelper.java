...
//        return null;
    ...
    public static boolean compare(Ingredient input, Ingredient other) {
        String[] inputString = Arrays.stream(input.getItems()).map(stack -> stack.getItem().getRegistryName().toString()).toArray(String[]::new);
        String[] otherString = Arrays.stream(other.getItems()).map(stack -> stack.getItem().getRegistryName().toString()).toArray(String[]::new);
