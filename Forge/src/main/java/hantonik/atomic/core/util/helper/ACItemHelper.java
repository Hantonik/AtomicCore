...
public final class ACItemHelper {
    ...
    public static JsonElement serializeItem(Item item) {
        return Util.getOrThrow(ForgeRegistries.ITEMS.getCodec().encodeStart(JsonOps.INSTANCE, item), IllegalStateException::new);
    }
    ...
    public static Item deserializeItem(JsonElement json) {
        return Util.getOrThrow(ForgeRegistries.ITEMS.getCodec().parse(JsonOps.INSTANCE, json), IllegalStateException::new);
    }
